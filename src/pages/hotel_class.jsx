...
const Hotel = () => {
  ...
  const loadAnnees = useCallback(async () => {
    try {
      ...
      console.log('📡 Années disponibles hotel:', response.data);
      if (response.data.success) {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      console.log('📡 Chargement des usagers hôtels...');
      
      // Récupérer les usagers hotel
      const usagersResponse = await axios.get(`${API_URL}/usagers/paiements/hotel`);
      console.log('📡 Réponse usagers hotel:', usagersResponse.data);
      ...
      if (usagersResponse.data.success && usagersResponse.data.usagers) {
        ...
        console.log(`✅ ${usagersData.length} usagers hôtels trouvés`);
      } else {
        console.warn('⚠️ Aucun usager hôtel trouvé dans la réponse');
        ...
        try {
          ...
          if (allUsagersResponse.data.success && allUsagersResponse.data.usagers) {
            usagersData = allUsagersResponse.data.usagers.filter(u => 
              u.type_usager === 'hotel' || u.type === 'hotel'
            );
            console.log(`✅ ${usagersData.length} usagers hôtels filtrés depuis la liste générale`);
          }
        } catch (err) {
      ...
      if (usagersData.length === 0) {
        ...
        setFilteredUsagers([]);
        setLoading(false);
        setApiError('Aucun usager hôtel trouvé dans la base de données');
        return;
      }
      ...
      setNotification({ 
        type: 'error', 
        message: '❌ Erreur de chargement des données hôtels' 
      });
    } finally {
      setLoading(false);
    }
        ...
        <div className="grandsurface-container">
          ...
          <div className="table-wrapper">
            {loading ? (
              ...
              <div className="empty">
                <p>Aucun usager hôtel trouvé pour la période sélectionnée</p>
                <button className="btn-refresh" onClick={refreshData}>
                  🔄 Rafraîchir
                </button>
              </div>
                          ...
                          <td>
                            {usager.etoiles ? (
                              <span style={{color: '#f59e0b'}}>
                                {'⭐'.repeat(usager.etoiles)}
                              </span>
                            ) : '-'}
                          </td>
                          <td>{usager.ravinala ? '🏆 Oui' : '-'}</td>
                          {[...Array(12)].map((_, i) => {
                            const mois = i + 1;
        ...
        <div className="modal-overlay" onClick={closeModal}>
          <div className="modal-container" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              <h3>🏨 {selectedUsager.denomination || selectedUsager.nom || 'Hôtel'}</h3>
                ...
                <div className="modal-row">
                  ...
                  <span>
                    {selectedUsager.etoiles ? (
                      <span style={{color: '#f59e0b'}}>
                        {'⭐'.repeat(selectedUsager.etoiles)}
                      </span>
                    ) : '-'}
                  </span>
                </div>
                <div className="modal-row">
                  <strong>Ravinala :</strong> <span>{selectedUsager.ravinala ? '🏆 Oui' : 'Non'}</span>
                </div>
                <div className="modal-row">
                  <strong>Nombre de chambres :</strong> <span>{selectedUsager.nombre_chambres || '-'}</span>
                </div>
                <div className="modal-row">
                  <strong>Catégorie :</strong> <span>{selectedUsager.categorie || selectedUsager.type_hotel || '-'}</span>
                </div>
              </div>
              <div className="modal-section">
