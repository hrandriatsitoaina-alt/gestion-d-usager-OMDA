...
const Tele = () => {
  ...
  const loadAnnees = useCallback(async () => {
    try {
      const response = await axios.get(`${API_URL}/paiements/annees-disponibles/media`);
      console.log('📡 Années disponibles media:', response.data);
      if (response.data.success) {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      console.log('📡 Chargement des usagers média (Télé/Radio)...');
      
      // Récupérer les usagers media
      const usagersResponse = await axios.get(`${API_URL}/usagers/paiements/media`);
      console.log('📡 Réponse usagers media:', usagersResponse.data);
      ...
      if (usagersResponse.data.success && usagersResponse.data.usagers) {
        ...
        console.log(`✅ ${usagersData.length} usagers média trouvés`);
      } else {
        console.warn('⚠️ Aucun usager média trouvé dans la réponse');
        ...
        try {
          ...
          if (allUsagersResponse.data.success && allUsagersResponse.data.usagers) {
            usagersData = allUsagersResponse.data.usagers.filter(u => 
              u.type_usager === 'media' || u.type === 'media' || 
              u.type_usager === 'tele_radio' || u.type === 'tele_radio'
            );
            console.log(`✅ ${usagersData.length} usagers média filtrés depuis la liste générale`);
          }
        } catch (err) {
      ...
      if (usagersData.length === 0) {
        ...
        setFilteredUsagers([]);
        setLoading(false);
        setApiError('Aucun usager média (Télé/Radio) trouvé dans la base de données');
        return;
      }
      ...
      const usagersWithYearData = usagersData.map(usager => {
        ...
        const moisPayesPourAnnee = paiements.filter(p => 
          ...
          (p.usager_type === 'media' || p.usager_type === 'tele_radio' || p.usager_type === usager.type_usager) &&
      ...
      console.error('❌ Erreur chargement données média:', error);
      setApiError(error.message || 'Erreur de chargement');
      setNotification({ 
        type: 'error', 
        message: '❌ Erreur de chargement des données média' 
      });
    } finally {
      setLoading(false);
    }
        ...
        <div className="grandsurface-container">
          ...
          <div className="table-wrapper">
            {loading ? (
              ...
              <div className="empty">
                <p>Aucun usager média (Télé/Radio) trouvé pour la période sélectionnée</p>
                <button className="btn-refresh" onClick={refreshData}>
                  🔄 Rafraîchir
                </button>
              </div>
                    ...
                    {currentUsagers.map((usager) => {
                      ...
                      return (
                        <tr key={usager.id} style={{borderLeft: `4px solid ${statusColor}`}}>
                          ...
                          <td>{usager.region || '-'}</td>
                          <td>{usager.frequence || '-'}</td>
                          <td>{usager.canal || usager.type_media || '-'}</td>
                          {[...Array(12)].map((_, i) => {
                            const mois = i + 1;
        ...
        <div className="modal-overlay" onClick={closeModal}>
          <div className="modal-container" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              <h3>📺 {selectedUsager.denomination || selectedUsager.nom || 'Média'}</h3>
            ...
            <div className="modal-body">
              <div className="modal-section">
                ...
                <div className="modal-row">
                  <strong>Fréquence :</strong> <span>{selectedUsager.frequence || '-'}</span>
                </div>
                <div className="modal-row">
                  <strong>Canal :</strong> <span>{selectedUsager.canal || selectedUsager.type_media || '-'}</span>
                </div>
                <div className="modal-row">
                  <strong>Type Média :</strong> <span>{selectedUsager.type_media || selectedUsager.categorie || '-'}</span>
                </div>
              </div>
              <div className="modal-section">
