...
const DateNight = () => {
  ...
  const loadAnnees = useCallback(async () => {
    try {
      ...
      console.log('📡 Années disponibles nightclub:', response.data);
      if (response.data.success) {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      console.log('📡 Chargement des usagers nightclub...');
      
      // Récupérer les usagers nightclub
      const usagersResponse = await axios.get(`${API_URL}/usagers/paiements/nightclub`);
      console.log('📡 Réponse usagers nightclub:', usagersResponse.data);
      ...
      if (usagersResponse.data.success && usagersResponse.data.usagers) {
        ...
        console.log(`✅ ${usagersData.length} usagers nightclub trouvés`);
      } else {
        console.warn('⚠️ Aucun usager nightclub trouvé dans la réponse');
        ...
        try {
          ...
          if (allUsagersResponse.data.success && allUsagersResponse.data.usagers) {
            usagersData = allUsagersResponse.data.usagers.filter(u => 
              u.type_usager === 'nightclub' || u.type === 'nightclub' || u.type_usager === 'night_club'
            );
            console.log(`✅ ${usagersData.length} usagers nightclub filtrés depuis la liste générale`);
          }
        } catch (err) {
          ...
          <div className="modal-container" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-body">
              <div className="modal-section">
                ...
                <div className="modal-row">
                  <strong>Jauge Max :</strong> <span>{selectedUsager.jauge_max || '-'}</span>
                </div>
                <div className="modal-row">
                  <strong>Type :</strong> <span>{selectedUsager.type || selectedUsager.categorie || '-'}</span>
                </div>
              </div>
              <div className="modal-section">
