...
// ============================================================
// GET - Paiements par type d'usager (CORRIGÉ)
// ============================================================
...
router.get('/usagers/paiements/:type', async (req, res) => {
  ...
  try {
    ...
    for (const usager of usagers.rows) {
      ...
      // ⭐ Récupérer le mois et l'année de création de l'usager
      let moisCreation = 1; // Par défaut Janvier
      let anneeCreation = currentYear;
      
      if (usager.created_at) {
        const creationDate = new Date(usager.created_at);
        moisCreation = creationDate.getMonth() + 1; // 1-12
        anneeCreation = creationDate.getFullYear();
      }
      
      // ⭐ Récupérer TOUS les paiements pour cet usager
      ...
      for (const p of paiements.rows) {
        ...
        if (!anneesPayes[annee]) {
          anneesPayes[annee] = [];
        }
        if (!moisPayesParAnnee[annee]) {
          moisPayesParAnnee[annee] = [];
        }
        anneesPayes[annee].push(mois);
        moisPayesParAnnee[annee].push(mois);
      }
      
      // ⭐ Construire resumeAnnees avec la bonne logique
      const resumeAnnees = [];
      for (let annee = currentYear - 1; annee <= currentYear + 1; annee++) {
        ...
        let anneeDebut = anneeCreation;
        
        // Si l'usager a été créé cette année, le début est son mois de création
        if (annee === anneeCreation) {
          moisDebutAnnee = moisCreation;
        } else if (annee > anneeCreation) {
          // Pour les années suivantes, le début est Janvier
          moisDebutAnnee = 1;
        }
        
        // ⭐ Calculer le nombre de mois depuis le début jusqu'à la fin de l'année
        const moisTotalAttendus = 12 - moisDebutAnnee + 1;
        
        // ⭐ Déterminer si l'année est complète
        let estComplete = false;
        
        if (nbMois > 0) {
          // Si l'usager a payé tous les mois depuis son début OU plus
          if (nbMois >= moisTotalAttendus) {
            estComplete = true;
          }
        }
        
        // ⭐ S'assurer que les mois payés sont bien dans la plage attendue
        const moisValides = moisPayes.filter(mois => mois >= moisDebutAnnee);
        const nbMoisValides = moisValides.length;
        
        // Recalculer la complétude avec les mois valides
        ...
        if (estComplete) {
          ...
          // Afficher 12/12 mais avec les mois correspondants
          const moisAffiches = moisTries.length > 0 ? ` (${affichageMois})` : '';
          affichage = `✅ 12/12${moisAffiches}`;
        ...
        resumeAnnees.push({
          annee,
          nbMois: nbMoisValides,
          moisTotalAttendus,
          moisDebut: moisDebutAnnee,
          estComplete,
          affichage,
          // Ajouter les informations de début pour le frontend
      ...
      result.push({
        ...
        montant_total: parseFloat(usager.montant_total) || 0,
        region: usager.region || 'N/A',
        adresse: usager.adresse || usager.adresse_siege || '',
        uniter: usager.uniter || 1,
        resumeAnnees: resumeAnnees,
        moisPayesParAnnee: moisPayesParAnnee,
        ...
        // ⭐ Informations de création pour le frontend
...
router.get('/usagers/nouveaux-ids/:type', async (req, res) => {
  ...
  try {
    const result = await pool.query(
      `SELECT id FROM ${tableName} WHERE created_at > NOW() - INTERVAL '24 hours'`
    );
    
    let ids = result.rows.map(row => row.id);
    
    const vusResult = await pool.query(
      `SELECT usager_id FROM usagers_vus WHERE usager_type = $1`,
      [type]
    );
...
router.get('/usagers/nouveaux-compteur', async (req, res) => {
  try {
    const nouveaux = {
      hotel: 0, 'grand-surface': 0, bus: 0, nightclub: 0, media: 0, occ: 0
    };
    ...
    for (const type of types) {
      const result = await pool.query(`
        ...
        AND NOT EXISTS (
          SELECT 1 FROM usagers_vus v 
          WHERE v.usager_id = u.id AND v.usager_type = $1
        )
      `, [type.name]);
    ...
    switch(type) {
      ...
      case 'OCC':
        ...
        query = `SELECT organisateurs, representant_par, genre_manifestation, artistes, date_evenement, lieu_evenement,
                        ...
                        confirmation_nom, date_signature, lieu_ajout, frais_dossier, region, uniter
      ...
      case 'Télé/Radio':
        ...
        break;
      default:
        return res.json({ success: false, exists: false, message: 'Type non supporté' });
    }
  ...
  try {
    const query = `
      SELECT 
        ...
        o.frais_dossier, o.organisateurs, o.representant_par, o.lieu_ajout, o.date_ajout,
...
router.get('/usagers/:id', async (req, res) => {
  ...
  try {
    const tables = [
      'usagers_hotel', 'usagers_magasin', 'usagers_media',
      'usagers_occasionnel', 'usagers_bus', 'usagers_nightclub'
    ];
    for (const table of tables) {
      const result = await pool.query(
...
router.get('/usagers/occasionnel/:id', async (req, res) => {
  ...
  try {
    ...
    if (result.rows.length === 0) {
      return res.status(404).json({ success: false, message: 'OCC non trouvé' });
    }
    res.json({ success: true, usager: result.rows[0] });
  } catch (error) {
...
router.get('/usagers/bus/:id', async (req, res) => {
  ...
  try {
    ...
    if (result.rows.length === 0) {
      return res.status(404).json({ success: false, message: 'Bus non trouvé' });
    }
    res.json({ success: true, usager: result.rows[0] });
  } catch (error) {
...
router.get('/usagers/nightclub/:id', async (req, res) => {
  ...
  try {
    ...
    if (result.rows.length === 0) {
      return res.status(404).json({ success: false, message: 'Night club non trouvé' });
    }
    res.json({ success: true, usager: result.rows[0] });
  } catch (error) {
...
// ============================================================
// POST - Ajouter un usager (SANS soit_total, montant_total, statut_paiement)
// ============================================================
  ...
  try {
    ...
    switch(type) {
      ...
      case 'OCC':
        tableName = 'usagers_occasionnel';
        insertData = { 
          organisateurs: data.organisateurs || '', representant_par: data.representantPar || '',
          genre_manifestation: data.genreManifestation || '', artistes: data.artistes || '',
          date_evenement: data.dateEvenement || null, lieu_evenement: data.lieuEvenement || '',
          representant_cin: data.representantCin || '', representant_cin_delivree: data.representantCinDelivree || null,
          representant_cin_lieu: data.representantCinLieu || '', adresse: data.adresse || '',
          telephone: data.telephone || '', domicile: data.domicile || '',
          confirmation_nom: data.confirmationNom || '', date_signature: data.dateSignature || null,
          lieu_ajout: data.lieuAjout || '', frais_dossier: parseFloat(data.fraisDossier) || 0,
          date_ajout: data.dateAjout || null,
          region: data.region || '', demandeur: data.organisateurs || '',
          denomination: data.genreManifestation || '', numero_dossier_global: data.numeroDossierGlobal || '',
      ...
      default: 
        return res.status(400).json({ success: false, message: 'Type d\'usager inconnu' });
    }
    ...
    if (userId) {
      ...
      if (counterResult.rows.length > 0) {
        const newCompteur = counterResult.rows[0].compteur + 1;
        await pool.query(
          `UPDATE compteurs_dossiers_utilisateurs 
           SET compteur = $1, updated_at = NOW() 
           WHERE id = $2`,
          [newCompteur, counterResult.rows[0].id]
        );
        console.log(`✅ Compteur ${type} pour utilisateur ${userId} incrémenté à ${newCompteur}`);
      } else {
        await pool.query(
        ...
        console.log(`✅ Nouveau compteur ${type} pour utilisateur ${userId} créé à 1`);
      }
    }
      ...
      if (data.otherArtistsDetail && data.otherArtistsDetail.length > 0) {
        for (const artist of data.otherArtistsDetail) {
          if (artist.nom && artist.nom.trim() !== '') {
            allArtists.push({ nom: artist.nom.trim(), prenom: artist.prenom || '', role: artist.role || 'Artiste participant' });
          }
        }
      }
...
router.put('/usagers/:id', async (req, res) => {
  ...
  try {
    let tableName = '';
    let typeValue = type_usager;
    
    if (typeValue === 'Télé/Radio' || typeValue === 'Media') {
      typeValue = 'Télé/Radio';
    }
    ...
    const commonFields = {
      ...
      representant_tel, representant_fonction, lieu_signature, date_signature,
      frais_dossier, montant_mensuel, uniter, adresse_siege: adresse,
    ...
    try {
      ...
      if (adminToken) {
        try {
          const userResult = await pool.query(`
            SELECT nom FROM utilisateurs WHERE role = 'super_admin' OR role = 'daf' LIMIT 1
          `);
          if (userResult.rows.length > 0) {
            modifiedBy = userResult.rows[0].nom;
          }
        } catch (err) {}
      }
      ...
      await pool.query(
        `INSERT INTO notifications (message, type, usager_id, created_at) 
         VALUES ($1, $2, $3, NOW())`,
        [`MODIFICATION: Usager "${denomination}" modifié par ${modifiedBy}`, 'update', parseInt(id)]
      );
    } catch (notifError) {
...
router.delete('/usagers/:id', async (req, res) => {
  ...
  try {
    ...
    await pool.query(`DELETE FROM paiements WHERE usager_id = $1 AND usager_type = $2`, [id, foundTable.type]);
    ...
    try {
      ...
      `, [
        ...
        userId,
        JSON.stringify({
          demandeur: usagerData.demandeur || 'Inconnu',
      ...
      console.error('❌ Erreur historique:', historyError.message);
    }
...
router.post('/usagers/marquer-vu', async (req, res) => {
  ...
  try {
    await pool.query(
      `INSERT INTO usagers_vus (usager_id, usager_type, vu_le) 
       VALUES ($1, $2, NOW()) 
       ON CONFLICT (usager_id, usager_type) DO NOTHING`,
      [usagerId, type]
    );
